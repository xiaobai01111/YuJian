  ...
  <dialog id="login_modal" class="modal">
    <div class="modal-box w-full max-w-sm p-8 rounded-3xl bg-base-100 shadow-2xl overflow-visible relative">
      ...
      <form v-if="isLogin" @submit.prevent="handleLogin" class="space-y-4">
        ...
        <div v-if="loginCaptcha.captchaImage || loginCaptcha.challenge" class="form-control">
          <div class="flex gap-2">
            <div class="flex-1 input input-bordered rounded-xl bg-base-200/50 h-12 flex items-center justify-center px-4 text-sm font-medium text-slate-700 overflow-hidden">
              <img
                v-if="loginCaptcha.captchaImage"
                :src="loginCaptcha.captchaImage"
                alt="登录验证码"
                class="max-h-10 w-full object-contain"
              />
              <span v-else>{{ loginCaptcha.challenge }}</span>
            </div>
            <button
              type="button"
              class="btn btn-outline rounded-xl h-12 px-3"
          ...
          <input
            ...
            type="text"
            placeholder="请输入验证码"
            class="input input-bordered rounded-xl bg-base-200/50 focus:bg-base-100 transition-colors h-12 mt-2"
            required
          />
        </div>
          ...
          <div class="form-control">
            <div class="relative">
              <input 
                ...
                type="password" 
                placeholder="密码 (8-32位，含大小写/数字/特殊字符)" 
                class="input input-bordered validator w-full pl-12 rounded-xl bg-base-200/50 focus:bg-base-100 transition-colors h-12"
                required 
                minlength="8"
                maxlength="32"
                pattern="^(?=.*[A-Z])(?=.*[a-z])(?=.*\\d)(?=.*[^A-Za-z\\d\\s]).{8,32}$"
                title="密码需为8-32位，且包含大小写字母、数字和特殊字符"
              />
              <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 absolute left-4 top-3.5 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
              </svg>
...
import { getLoginCaptcha, sendRegisterEmailCode } from '@/api/auth'
...
const handleLogin = async () => {
  if ((loginCaptcha.captchaImage || loginCaptcha.challenge) && !loginForm.captchaCode.trim()) {
  ...
  try {
    ...
    loginCaptcha.challenge = ''
    loginCaptcha.captchaImage = ''
  } catch (error: unknown) {
    const message = (error as ApiErrorLike)?.message || '登录失败，请检查用户名和密码'
...
const loadLoginCaptcha = async () => {
  ...
  try {
    ...
    loginForm.captchaCode = ''
    loginCaptcha.challenge = res.challenge || ''
    loginCaptcha.captchaImage = res.captchaImage || ''
    loginCaptcha.expireSeconds = res.expireSeconds
  } catch (error: unknown) {
    errorMsg.value = (error as ApiErrorLike)?.message || '获取验证码失败'
  } finally {
