...
COMMENT ON COLUMN users.credit_score IS '信用分，默认100，用于市集交易';
...
COMMENT ON COLUMN sys_roles.role_key IS '权限字符，用于 Sa-Token @SaCheckRole';
...
COMMENT ON COLUMN posts.lost_time IS '丢失/拾取时间，用于失物招领';
...
COMMENT ON COLUMN anonymous_mappings.user_id_encrypted IS '加密后的用户ID，使用 KMS 加密';
