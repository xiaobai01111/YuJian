...
CREATE TABLE sys_upload_policies (
    id BIGSERIAL PRIMARY KEY,
...
CREATE TABLE sys_oper_log (
    ...
    request_body_digest VARCHAR(64),
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
COMMENT ON COLUMN users.email_hash IS '邮箱SHA-256哈希（用于等值检索）';
COMMENT ON COLUMN users.edu_email_hash IS '教育邮箱SHA-256哈希（用于等值检索）';
COMMENT ON COLUMN users.phone_hash IS '手机号SHA-256哈希（用于等值检索）';

CREATE INDEX idx_users_dept_id ON users(dept_id);
CREATE INDEX idx_users_status ON users(status);
CREATE INDEX idx_users_deleted ON users(deleted);
CREATE INDEX idx_users_email_hash ON users(email_hash);
CREATE INDEX idx_users_edu_email_hash ON users(edu_email_hash);
...
COMMENT ON COLUMN files.asset_type IS '归档资产类型(file/gallery/resource)';
COMMENT ON COLUMN files.public_key IS '对外访问标识';

CREATE INDEX idx_files_user ON files(user_id);
CREATE INDEX idx_files_target ON files(target_type, target_id);
CREATE INDEX idx_files_asset_type ON files(asset_type);
CREATE INDEX idx_files_orphan_marked ON files(status, orphan_marked_at);
CREATE UNIQUE INDEX ux_files_public_key ON files(public_key);
