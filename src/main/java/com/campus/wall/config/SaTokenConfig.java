...
public class SaTokenConfig implements WebMvcConfigurer {
    ...
    public void addInterceptors(@NonNull InterceptorRegistry registry) {
        registry.addInterceptor(new SaInterceptor(handle -> {
            ...
            if (isActuatorEndpoint(uri)) {
                checkBlocklist(method, uri);
                checkActuatorAccess(uri);
                return;
            }
    ...
    private boolean isActuatorEndpoint(String uri) {
        return "/actuator".equals(uri) || (uri != null && uri.startsWith("/actuator/"));
    }

    private void checkActuatorAccess(String uri) {
        if (securityProperties.isActuatorAllowAnonymousHealth() && isHealthEndpoint(uri)) {
            return;
        }
        StpUtil.checkLogin();
        if (securityProperties.isActuatorRequireSuperAdmin()
                && !StpUtil.hasRole(SecurityUtil.getSuperAdminRoleKey())) {
            throw new BusinessException(ResultCode.FORBIDDEN, "无权访问运维端点");
        }
    }

    private boolean isHealthEndpoint(String uri) {
        return "/actuator/health".equals(uri) || "/actuator/health/".equals(uri)
                || (uri != null && uri.startsWith("/actuator/health/"));
    }
