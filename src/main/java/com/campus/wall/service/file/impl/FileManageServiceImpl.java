...
public class FileManageServiceImpl implements FileManageService {
    ...
    private PageResult<FileVO> queryByAssetType(FileQueryDTO query, String assetType) {
        ...
        applySensitiveTargetScope(wrapper);
    ...
    public void updateVisibility(Long fileId, String visibility) {
        if (fileId == null) {
            throw new BusinessException(ResultCode.BAD_REQUEST, "文件 ID 不能为空");
        }
        if (!StringUtils.hasText(visibility) || !FileVisibility.isValidCode(visibility)) {
            throw new BusinessException(ResultCode.BAD_REQUEST, "无效的文件可见性");
        }
        FileRecord record = fileRecordMapper.selectById(fileId);
        if (record == null) {
            throw new BusinessException(ResultCode.NOT_FOUND, "文件不存在");
        }
        FileVisibility normalized = FileVisibility.fromCode(visibility);
        if (record.getVisibility() != null && normalized.getCode().equalsIgnoreCase(record.getVisibility())) {
            return;
        }
        record.setVisibility(normalized.getCode());
    ...
    private void applySensitiveTargetScope(LambdaQueryWrapper<FileRecord> wrapper) {
        if (wrapper == null) {
            return;
        }
        // 身份材料不纳入通用资产查询，避免与常规文件/图库/资源同域暴露。
        wrapper.ne(FileRecord::getTargetType, TARGET_TYPE_ID_CARD);
    }
