...
public class RoleServiceImpl implements RoleService {
    ...
    public List<UserVO> getRoleUsers(Long roleId) {
        ...
        return userMapper.selectBatchIds(userIds).stream()
            .filter(Objects::nonNull)
            .filter(user -> canAccessByScope(user, operatorId, scope))
            .map(this::toRoleUserVO)
            .collect(Collectors.toList());
    }

    private boolean canAccessByScope(User user, Long operatorId, DataScopeService.DataScope scope) {
        if (user == null) {
            return false;
        }
        ...
        if (scope.isAllowSelf() && operatorId != null && operatorId.equals(user.getId())) {
            return true;
        }
        List<Long> scopedDeptIds = scope.getScopedDeptIds();
        return scopedDeptIds != null
            && !scopedDeptIds.isEmpty()
            && user.getDeptId() != null
            && scopedDeptIds.contains(user.getDeptId());
    }

    private UserVO toRoleUserVO(User user) {
        UserVO vo = new UserVO();
        BeanUtils.copyProperties(user, vo);
        // 角色用户列表仅用于关联确认，不返回可识别联系信息。
        vo.setEmail(null);
        vo.setPhone(null);
        return vo;
    }
