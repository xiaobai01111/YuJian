...
public class OnlineUserServiceImpl implements OnlineUserService {
    ...
    public void kickoutByToken(String token) {
        String normalizedToken = normalizeTokenValue(token);
        revokeRefreshTokenByAccessToken(normalizedToken);
        StpUtil.kickoutByTokenValue(normalizedToken);
        removeOnlineToken(normalizedToken);
    }
    ...
    private void revokeRefreshTokenByAccessToken(String token) {
        if (!StringUtils.hasText(token)) {
            return;
        }
        try {
            ...
            if (refreshToken instanceof String refresh && StringUtils.hasText(refresh)) {
                String normalizedRefresh = refresh.trim();
                redisTemplate.delete(LOGIN_REFRESH_TOKEN_PREFIX + normalizedRefresh);
                if (userId != null) {
                    redisTemplate.opsForSet().remove(LOGIN_REFRESH_USER_INDEX_PREFIX + userId, normalizedRefresh);
                }
            }
        } catch (Exception ignored) {
            // 回收刷新令牌失败不阻断踢下线主流程
