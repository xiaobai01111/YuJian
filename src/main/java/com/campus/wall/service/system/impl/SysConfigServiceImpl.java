...
public class SysConfigServiceImpl implements SysConfigService {
    ...
    public void updateEmailAllowedDomains(List<String> domains) {
        try {
            List<String> normalizedDomains = domains != null ? domains : new ArrayList<>();
            String value = objectMapper.writeValueAsString(normalizedDomains);
            SysConfig config = sysConfigMapper.selectOne(
                new LambdaQueryWrapper<SysConfig>().eq(SysConfig::getConfigKey, SysConfigKeys.EMAIL_ALLOWED_DOMAINS)
            );
            List<String> beforeDomains = config == null
                ? List.of("edu.cn")
                : parseConfigList(config.getConfigValue(), List.of("edu.cn"));
            if (config == null) {
            ...
            auditConfigChange("email_domains_update", SysConfigKeys.EMAIL_ALLOWED_DOMAINS, beforeDomains, normalizedDomains);
        } catch (Exception e) {
    ...
    public void updateStudentIdWhitelist(List<String> studentIds) {
        try {
            List<String> normalizedStudentIds = studentIds != null ? studentIds : new ArrayList<>();
            String value = objectMapper.writeValueAsString(normalizedStudentIds);
            SysConfig config = sysConfigMapper.selectOne(
                new LambdaQueryWrapper<SysConfig>().eq(SysConfig::getConfigKey, SysConfigKeys.STUDENT_ID_WHITELIST)
            );
            List<String> beforeStudentIds = config == null
                ? List.of()
                : parseConfigList(config.getConfigValue(), List.of());
            if (config == null) {
                config = new SysConfig();
            ...
            auditConfigChange(
                "student_whitelist_update",
                SysConfigKeys.STUDENT_ID_WHITELIST,
                beforeStudentIds,
    ...
    public void updateSmtpConfig(Map<String, Object> config) {
        Map<String, Object> beforeSnapshot = buildSmtpAuditSnapshot();
        // P2-1: SMTP配置强校验
        ...
        afterSnapshot.put("host", host);
        afterSnapshot.put("port", port);
        afterSnapshot.put("username", username);
        afterSnapshot.put("fromName", fromName);
        afterSnapshot.put("ssl", "true".equalsIgnoreCase(ssl));
        boolean beforePasswordConfigured = Boolean.TRUE.equals(beforeSnapshot.get("passwordConfigured"));
        afterSnapshot.put("passwordConfigured", passwordChanged || beforePasswordConfigured);
        afterSnapshot.put("passwordChanged", passwordChanged);
        auditConfigChange("smtp_update", "smtp.config", beforeSnapshot, afterSnapshot);
    }
    ...
    private void auditConfigChange(String action, String reason, Object beforeValue, Object afterValue) {
        operLogService.log(resolveOperatorId(), null, "sys_config", null,
            action, reason, beforeValue, afterValue, null);
    }
    ...
    private List<String> parseConfigList(String value, List<String> defaultValue) {
        if (value == null || value.isBlank()) {
            return defaultValue;
        }
        try {
    ...
    private Map<String, Object> buildSmtpAuditSnapshot() {
        Map<String, Object> snapshot = new HashMap<>();
        snapshot.put("host", getConfigValue(SysConfigKeys.SMTP_HOST));
        snapshot.put("port", parseIntOrDefault(getConfigValue(SysConfigKeys.SMTP_PORT), 465));
        snapshot.put("username", getConfigValue(SysConfigKeys.SMTP_USERNAME));
        snapshot.put("fromName", getConfigValue(SysConfigKeys.SMTP_FROM_NAME));
        snapshot.put("ssl", "true".equalsIgnoreCase(getConfigValue(SysConfigKeys.SMTP_SSL)));
        String encryptedPassword = getConfigValue(SysConfigKeys.SMTP_PASSWORD);
        snapshot.put("passwordConfigured", encryptedPassword != null && !encryptedPassword.isBlank());
        return snapshot;
    }

    private Map<String, Object> summarizeTemplates(Map<String, Object> templates) {
        Map<String, Object> summary = new HashMap<>();
        if (templates == null || templates.isEmpty()) {
            summary.put("templateCount", 0);
            summary.put("templateKeys", List.of());
            return summary;
        }
        summary.put("templateCount", templates.size());
        summary.put("templateKeys", templates.keySet().stream().sorted().toList());
        return summary;
    }
    ...
    public void updateEmailTemplates(Map<String, Object> templates) {
        ...
        try {
            Map<String, Object> beforeSummary = summarizeTemplates(getEmailTemplates());
            ...
            auditConfigChange(
                ...
                SysConfigKeys.EMAIL_TEMPLATES,
                beforeSummary,
                summarizeTemplates(templates)
            );
        } catch (Exception e) {
