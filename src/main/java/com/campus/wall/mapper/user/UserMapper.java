...
public interface UserMapper extends BaseMapper<User> {
    ...
    @Results(value = {
        @Result(column = "email", property = "email", typeHandler = EncryptedStringTypeHandler.class),
        @Result(column = "edu_email", property = "eduEmail", typeHandler = EncryptedStringTypeHandler.class),
        @Result(column = "phone", property = "phone", typeHandler = EncryptedStringTypeHandler.class),
        @Result(column = "student_id", property = "studentId", typeHandler = EncryptedStringTypeHandler.class)
    })
    ...
    @Insert("""
        <script>
        INSERT INTO users (username, password, nickname, email, email_hash, phone, phone_hash, verify_status, status, credit_score)
        VALUES
        <foreach collection="users" item="item" separator=",">
          (#{item.username}, #{item.password}, #{item.nickname},
           #{item.email,typeHandler=com.campus.wall.config.typehandler.EncryptedStringTypeHandler},
           #{item.emailHash},
           #{item.phone,typeHandler=com.campus.wall.config.typehandler.EncryptedStringTypeHandler},
           #{item.phoneHash},
    ...
    @Select("""
        ...
        SELECT * FROM users
        WHERE deleted = 1
        <if test="(scopedDeptIds != null and scopedDeptIds.size > 0) or (allowSelf != null and allowSelf and operatorId != null)">
            AND (
                <if test="scopedDeptIds != null and scopedDeptIds.size > 0">
                    dept_id IN
                    <foreach collection="scopedDeptIds" item="deptId" open="(" separator="," close=")">
                        #{deptId}
                    </foreach>
                </if>
                <if test="scopedDeptIds != null and scopedDeptIds.size > 0 and allowSelf != null and allowSelf and operatorId != null">
                    OR
                </if>
                <if test="allowSelf != null and allowSelf and operatorId != null">
                    id = #{operatorId}
                </if>
            )
        </if>
    ...
    List<User> selectDeletedUsersAfter(@Param("username") String username,
                                       ...
                                       @Param("scopedDeptIds") List<Long> scopedDeptIds,
                                       @Param("allowSelf") Boolean allowSelf,
    ...
    @Select("""
        ...
        <if test="phone != null and phone != ''">
            AND (phone_hash = #{phoneHash} OR phone = #{phone})
        </if>
        <if test="loginDateStart != null">
            AND login_date <![CDATA[>=]]> #{loginDateStart}
        </if>
