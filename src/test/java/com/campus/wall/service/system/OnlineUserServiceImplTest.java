...
class OnlineUserServiceImplTest {
    ...
    @Test
    void kickoutByToken_revokesBoundRefreshToken() {
        StpUtil.login(3L);
        String token = StpUtil.getTokenValue();
        StpUtil.getStpLogic()
            .getTokenSessionByToken(token)
            .set(SecurityConstants.TOKEN_SESSION_REFRESH_TOKEN, "refresh-xyz");

        when(redisTemplate.delete("campus:login:refresh:refresh-xyz")).thenReturn(true);
        when(setOperations.remove(CacheConstants.ONLINE_TOKENS, token)).thenReturn(1L);

        onlineUserService.kickoutByToken(token);

        verify(redisTemplate).delete("campus:login:refresh:refresh-xyz");
        verify(setOperations).remove(CacheConstants.ONLINE_TOKENS, token);
    }
}
